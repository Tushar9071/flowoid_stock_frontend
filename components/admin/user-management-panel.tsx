...
export function UserManagementPanel({ showLocalAction = false }: { showLocalAction?: boolean }) {
  ...
  const [statusFilterDropdown, setStatusFilterDropdown] = useState<string[]>([]);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return tenantScopedUsers.filter(user => {
      ...
      const matchesRole = roleFilter.length === 0 || roleFilter.includes(roleName);
      const userStatus = user.isActive ? 'Active' : 'Inactive';
      const matchesStatusDropdown = statusFilterDropdown.length === 0 || statusFilterDropdown.includes(userStatus);

      return matchesStatus && matchesSearch && matchesRole && matchesStatusDropdown;
    });
  }, [activeFilter, search, tenantScopedUsers, roleFilter, statusFilterDropdown]);
  ...
  const handleSaveUser = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        if (!editingUser) {
          const defaultPermissions = await PermissionService.assignDefaultOwnerPermissions(response.data.id);
          if (!defaultPermissions.success) {
            toast.error(defaultPermissions.error?.message || 'Owner created, but default permissions were not assigned');
          }
        }
        toast.success(editingUser ? 'User updated successfully' : 'Owner created with default permissions');
        closeModal();
        await loadData();
      } else {
                ...
                <TableRow className="theme-table-header">
                  ...
                  <TableHead 
                    ...
                    filterOptions={[
                      ...
                      { label: 'Flowoid Admin', value: 'Flowoid Admin' },
                  ...
                  <TableHead 
                    ...
                    filterValues={statusFilterDropdown}
                    onFilterChange={setStatusFilterDropdown}
                  >
                    Status
                  </TableHead>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0F172A]/70 p-3 backdrop-blur-sm sm:p-4">
          <Card className="theme-modal-panel flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden">
            ...
            Backend supports ADMIN-managed OWNER accounts. Permissions are assigned from Owner Permissions.
                </p>
              </div>
            ...
            <form onSubmit={handleSaveUser} className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Detail label="Backend Role" value="OWNER" />
