...
export default function DashboardRolesPage() {
  ...
  const canManageRoles = isSuperAdminRole(role) && (isFullAccess || hasPermission('roles.read'));
    ...
    return (
      <DashboardLayout title="Roles & Permissions">
        <div className="mx-auto mt-10 max-w-2xl rounded-xl border border-[#e5e7eb] bg-white p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-[#6b7280]">Role and permission assignment is available only in the admin panel.</p>
        </div>
      </DashboardLayout>
    );
  }
